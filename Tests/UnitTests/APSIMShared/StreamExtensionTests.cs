...
namespace UnitTests.APSIMShared
{
    ...
    public class StreamExtensionTests
    {
        ...
        {
            Assert.That(StreamExtensions.Equal(null, null), Is.True);
            Assert.That(StreamExtensions.Equal(new byte[0], null), Is.False);
            Assert.That(StreamExtensions.Equal(null, new byte[0]), Is.False);
            Assert.That(StreamExtensions.Equal(new byte[0], new byte[1]), Is.False);
            Assert.That(StreamExtensions.Equal(new byte[2] { 0, 1 }, new byte[2] { 0, 2 }), Is.False);
            Assert.That(StreamExtensions.Equal(new byte[2] { 64, 128 }, new byte[2] { 64, 128 }), Is.True);
        }
