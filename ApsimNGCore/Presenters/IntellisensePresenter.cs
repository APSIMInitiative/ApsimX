namespace UserInterface.Presenters
{
    ...
    public class IntellisensePresenter
    {
        ...
        /// <returns></returns>
        public List<NeedContextItemsArgs.ContextItem> GenerateScriptCompletionsAdHoc(string code, int offset, bool controlSpace = false)
        {
            // todo: might be simpler to use the Recommendation API,
            // but this will require construction of a workspace.
            SyntaxTree tree = SyntaxFactory.ParseSyntaxTree(code);
            CSharpCompilation compilation = CSharpCompilation.Create("MyCompilation").AddReferences(GetReferences()).AddSyntaxTrees(tree);
            SemanticModel model= compilation.GetSemanticModel(tree);
            
            // Get IdentifierNameSyntax corresponding to identifier at the given offset.
            SyntaxNode identifier = tree.GetRoot().FindToken(offset).Parent;

            // GetTypeSymbol corresponding to identifier at the given offset.
            ITypeSymbol type;
            if (identifier is MemberAccessExpressionSyntax memberAccess)
                type = model.GetTypeInfo(memberAccess.Expression).Type;
            else
                type = model.GetTypeInfo(identifier).Type;
            
            // Get symbols for accessible members on the above TypeSymbol.
            IEnumerable<ISymbol> symbols = model.LookupSymbols(offset, container: type, includeReducedExtensionMethods: true);
            List<NeedContextItemsArgs.ContextItem> contextItems = symbols.Select(symbol =>
            {
                IEnumerable<AttributeData> attributes = symbol.GetAttributes();

                var contextItem = new NeedContextItemsArgs.ContextItem()
                {
                    Name = symbol.Name,
                    Units = GetUnits(symbol),
                    Descr = GetDescription(symbol),
                    IsMethod = symbol.Kind == SymbolKind.Method,
                    IsEvent = symbol.Kind == SymbolKind.Event,
                    IsProperty = symbol.Kind == SymbolKind.Property,
                    IsWriteable = symbol is IPropertySymbol prop && !prop.IsReadOnly,
                    IsChildModel = false,
                };
                if (symbol is IMethodSymbol method)
                {
                    contextItem.ParamString = GetMethodParamString(method);
                    contextItem.TypeName = method.ReturnType.Name;
                }
                return contextItem;
            }).ToList();
            //view.Populate(contextItems.ToList());
            return contextItems;
        }

        private string GetMethodParamString(IMethodSymbol method)
        {
            return string.Join(", ", method.Parameters.Select(p => p.Name));
        }

        private string GetUnits(ISymbol symbol)
        {
            IEnumerable<AttributeData> attributes = symbol.GetAttributes();
            AttributeData unitsAttribute = attributes.FirstOrDefault(a => a.AttributeClass.Name == "UnitsAttribute");
            if (unitsAttribute != null)
                return unitsAttribute.ConstructorArguments.ElementAt(0).Value.ToString();
        ...
        {
            string summary = symbol.GetDocumentationCommentXml();
            if (!string.IsNullOrWhiteSpace(summary))
            {
                XmlDocument doc = new XmlDocument();
                // fixme - temp hack to work around multiple root
                // elements error.
                doc.LoadXml($"<test>{summary}</test>");

                string xPath = "//summary[1]";
                XmlNode summaryNode = doc.SelectSingleNode(xPath);
                if (summaryNode != null && summaryNode.ChildNodes.Count > 0)
                {
            ...
            IEnumerable<AttributeData> attributes = symbol.GetAttributes();
            AttributeData descriptionAttribute = attributes.FirstOrDefault(a => a.AttributeClass.Name == "DescriptionAttribute");
            if (descriptionAttribute != null && descriptionAttribute.ConstructorArguments.Count() > 0)
                return descriptionAttribute.ConstructorArguments.ElementAt(0).Value?.ToString();
