    ...
    public class Phenology : Model, IPhenology
    {
        ...
        public void SetToStage(double newStage)
        {
            ...
            if (newStage < Stage)
            {
                ...
                {
                    if (!(phase is IPhaseWithTarget) && !(phase is GotoPhase) && !(phase is EndPhase) && !(phase is PhotoperiodPhase) && !(phase is LeafDeathPhase) && !(phase is DAWSPhase) && !(phase is StartPhase) && !(phase is GrazeAndRewind) && !(phase is OnDatePhase))
