...
namespace Models.WholeFarm
{
	...
	public class RuminantMusterGroup: Model
	{
		...
		private void OnStartOfSimulation(object sender, EventArgs e)
		{
			...
			if (pasture == null)
			{
				Summary.WriteWarning(this, $"Could not find manage pasture activity named \"{ManagedPastureName}\" for {this.Name}");
				return;
			}

			// check if GUID has been set to ensure the pasture is currently managed
			if (pasture.ID == null)
			{
				Summary.WriteWarning(this, $"Managed pasture {this.Name} has not beed initialised with a unique ID");
				return;
			}
