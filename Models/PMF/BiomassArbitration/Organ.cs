        ...
        public void Document(List<ITag> tags, int headingLevel, int indent)
        {
            ...
            if (DMReallocFac is Constant)
            {
                if ((DMReallocFac as Constant).Value() == 0)
                    tags.Add(new Paragraph(Name + " does not reallocate DM when senescence of the organ occurs.", indent));
                else
                    tags.Add(new Paragraph(Name + " will reallocate " + (DMReallocFac as Constant).Value() * 100 + "% of DM that senesces each day.", indent));
            }
            else
            {
            ...
            if (DMRetransFac is Constant)
            {
                if ((DMRetransFac as Constant).Value() == 0)
                    tags.Add(new Paragraph(Name + " does not retranslocate non-structural DM.", indent));
                else
                    tags.Add(new Paragraph(Name + " will retranslocate " + (DMRetransFac as Constant).Value() * 100 + "% of non-structural DM each day.", indent));
            }
            else
            {
            ...
            if (NReallocFac is Constant)
            {
                if ((NReallocFac as Constant).Value() == 0)
                    tags.Add(new Paragraph(Name + " does not reallocate N when senescence of the organ occurs.", indent));
                else
                    tags.Add(new Paragraph(Name + " can reallocate up to " + (NReallocFac as Constant).Value() * 100 + "% of N that senesces each day if required by the plant arbitrator to meet N demands.", indent));
            }
            else
            {
            ...
            if (NRetransFac is Constant)
            {
                if ((NRetransFac as Constant).Value() == 0)
                    tags.Add(new Paragraph(Name + " does not retranslocate non-structural N.", indent));
                else
                    tags.Add(new Paragraph(Name + " can retranslocate up to " + (NRetransFac as Constant).Value() * 100 + "% of non-structural N each day if required by the plant arbitrator to meet N demands.", indent));
            }
            else
            {
            ...
            if (SenRate is Constant)
            {
                if ((SenRate as Constant).Value() == 0)
                    tags.Add(new Paragraph(Name + " has senescence parameterised to zero so all biomass in this organ will remain alive.", indent));
                else
                    tags.Add(new Paragraph(Name + " senesces " + (SenRate as Constant).Value() * 100 + "% of its live biomass each day, moving the corresponding amount of biomass from the live to the dead biomass pool.", indent));
            }
            else
            {
            ...
            if (DetRate is Constant)
            {
                if ((DetRate as Constant).Value() == 0)
                    tags.Add(new Paragraph(Name + " has detachment parameterised to zero so all biomass in this organ will remain with the plant until a defoliation or harvest event occurs.", indent));
                else
                    tags.Add(new Paragraph(Name + " detaches " + (DetRate as Constant).Value() * 100 + "% of its live biomass each day, passing it to the surface organic matter model for decomposition.", indent));
            }
            else
            {
