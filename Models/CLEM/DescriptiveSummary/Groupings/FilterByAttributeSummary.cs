    ...
    public class FilterByAttributeSummary : FilterSummaryBase<FilterByAttribute>
    {
        ...
        public override string FilterString(bool htmltags)
        {
            ...
            if (model.FilterStyle == AttributeFilterStyle.Exists | trueFalse)
            {
                ...
                filterWriter.Write($" attribute {CLEMModel.DisplaySummaryValueSnippet(model.AttributeTag, "No tag", htmlTags: htmltags, entryStyle: HTMLSummaryStyle.Filter)}");
            }
            else
            {
                filterWriter.Write($" Attribute {CLEMModel.DisplaySummaryValueSnippet(model.AttributeTag, "No tag", htmlTags: htmltags, entryStyle: HTMLSummaryStyle.Filter)}");
                filterWriter.Write($" {CLEMModel.DisplaySummaryValueSnippet(model.OperatorToSymbol(), "Unknown operator", htmlTags: htmltags, entryStyle: HTMLSummaryStyle.Filter)}");
                filterWriter.Write($" {CLEMModel.DisplaySummaryValueSnippet(model.Value?.ToString(), "No value", htmlTags: htmltags, entryStyle: HTMLSummaryStyle.Filter)}");
            }
