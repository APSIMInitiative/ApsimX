namespace Models.Climate
{
    ...
    public class Weather : Model, IWeather, IReferenceExternalFiles
    {
        ...
        /// <summary>
        /// Today's raw data straight from the met file.
        /// This will be null when the simulation is not being run.
        /// </summary>
        private object[] todaysRawData;
        ...
        private DailyMetDataFromFile GetMetData(DateTime date)
        {
            ...
            try
            {
                todaysRawData = this.reader.GetNextLineOfData();
            }
            ...
            if (date != this.reader.GetDateFromValues(todaysRawData))
            ...
            if (this.radiationIndex != -1)
                readMetData.Radn = Convert.ToSingle(todaysRawData[this.radiationIndex], CultureInfo.InvariantCulture);
            else
                readMetData.Radn = this.reader.ConstantAsDouble("radn");

            if (this.maximumTemperatureIndex != -1)
                readMetData.MaxT = Convert.ToSingle(todaysRawData[this.maximumTemperatureIndex], CultureInfo.InvariantCulture);
            else
                readMetData.MaxT = this.reader.ConstantAsDouble("maxt");

            if (this.minimumTemperatureIndex != -1)
                readMetData.MinT = Convert.ToSingle(todaysRawData[this.minimumTemperatureIndex], CultureInfo.InvariantCulture);
            else
                readMetData.MinT = this.reader.ConstantAsDouble("mint");

            if (this.rainIndex != -1)
                readMetData.Rain = Convert.ToSingle(todaysRawData[this.rainIndex], CultureInfo.InvariantCulture);
            else
            ...
            if (this.evaporationIndex == -1)
                ...
                readMetData.PanEvap = Convert.ToSingle(todaysRawData[this.evaporationIndex], CultureInfo.InvariantCulture);

            if (this.rainfallHoursIndex == -1)
                readMetData.RainfallHours = double.NaN;
            else
                readMetData.RainfallHours = Convert.ToSingle(todaysRawData[this.rainfallHoursIndex], CultureInfo.InvariantCulture);

            if (this.vapourPressureIndex == -1)
                readMetData.VP = Math.Max(0, MetUtilities.svp(readMetData.MinT));
            else
                readMetData.VP = Convert.ToSingle(todaysRawData[this.vapourPressureIndex], CultureInfo.InvariantCulture);

            if (this.windIndex == -1)
                readMetData.Wind = 3.0;
            else
                readMetData.Wind = Convert.ToSingle(todaysRawData[this.windIndex], CultureInfo.InvariantCulture);

            if (co2Index != -1)
                readMetData.CO2 = Convert.ToDouble(todaysRawData[co2Index], CultureInfo.InvariantCulture);
            ...
            if (this.DiffuseFractionIndex == -1)
            {
                ...
                readMetData.DiffuseFraction = Convert.ToSingle(todaysRawData[this.DiffuseFractionIndex], CultureInfo.InvariantCulture);
            ...
            if (this.dayLengthIndex == -1)  // Daylength is not a column - check for a constant
            {
                ...
                readMetData.DayLength = Convert.ToSingle(todaysRawData[this.dayLengthIndex], CultureInfo.InvariantCulture);
        ...
        public double GetValue(string columnName)
        {
            ...
            return Convert.ToDouble(todaysRawData[columnIndex], CultureInfo.InvariantCulture);
        }
