...
namespace Models.Core.ApsimFile
{
    ...
    public class ConverterUtilities
    {
        ...
        /// <summary>
        /// Get a child model with the specified model name.
        /// </summary>
        /// <param name="parent">Parent model.</param>
        /// <param name="modelName">Name of child to find.</param>
        /// <returns>The child token or null if not found.</returns>
        internal static JToken ChildModelWithName(JToken parent, string modelName)
        {
            foreach (var child in ChildModels(parent))
                if (child["Name"].ToString() == modelName)
                    return child;
        ...
        /// <summary>
        /// Find all child models recursively of the specified type.
        /// </summary>
        /// <param name="parent">Parent model.</param>
        /// <param name="modelTypeName">Type name of model to find.</param>
        /// <returns>Returns a list of matches. Never null.</returns>
        internal static List<JToken> FindAllModelsOfType(JToken parent, string modelTypeName)
        {
            List<JToken> matches = new List<JToken>();

            if (GetModelTypeName(parent) == modelTypeName)
                matches.Add(parent);

            foreach (var child in ChildModels(parent))
                matches.AddRange(FindAllModelsOfType(child, modelTypeName));

            return matches;
        }

        /// <summary>
        /// Get the APSIM model type name e.g. Plant for the specified model.
        /// </summary>
        /// <param name="modelToken">The token representing an APSIM model.</param>
        /// <returns>The model type of null if not a model.</returns>
        internal static string GetModelTypeName(JToken modelToken)
        {
            if (modelToken is JObject)
            {
                var objType = modelToken["$type"];
                if (objType != null)
                {
                    var typeName = objType.ToString().Replace(", Models", "");
                    var lastPeriod = typeName.LastIndexOf('.');
                    if (lastPeriod != -1)
                        return typeName.Substring(lastPeriod + 1);
                }
            }
            return null;
        }
        ...
        internal static JToken ParentModel(JToken modelToken)
        {
            var obj = modelToken.Parent;
            while (obj != null)
            {
                if (GetModelTypeName(obj) != null)
                    return obj;
        ...
        /// <summary>
        /// Return a list of all child APSIM model tokens.
        /// </summary>
        /// <param name="modelToken">The APSIM model token.</param>
        /// <returns>A list of child APSIM models. Never returns null.</returns>
        internal static List<JToken> ChildModels(JToken modelToken)
        {
            List<JToken> returnTokens = new List<JToken>();

            var children = modelToken["Children"];
            if (children != null)
            {
                foreach (var child in children)
                    returnTokens.Add(child);
            }
            return returnTokens;
        }
        ...
        internal static void RenameProperty(JToken modelToken, string propertyName, string newPropertyName)
        {
        ...
        /// <param name="name">The name of the constant function</param>
        /// <param name="fixedValue">The fixed value of the constant function</param>
        internal static void AddConstantFuntionIfNotExists(JToken modelToken, string name, string fixedValue)
        {
            if (ChildModelWithName(modelToken, name) == null)
            {
