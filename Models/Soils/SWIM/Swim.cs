    ...
    public class Swim3 : Model, ISoilWater
    {
        ...
        private void RemoveFromRainfall(double amount)
        {
            if (amount > 0.0)
            {
                ...
                double start_timestep = Time(clock.Today.Year, clock.Today.DayOfYear, time_mins);
                int start = 0;
        ...
        private void CalcEvapVariables()
        {
            ...
            double timeMins = Time(clock.Today.Year, clock.Today.Day, timeOfDay);
        ...
        private void CNRunoff()
        {
            ...
            double startOfDay = Time(clock.Today.Year, clock.Today.Day, TimeToMins(apsim_time));
            double endOfDay = Time(clock.Today.Year, clock.Today.Day, TimeToMins(apsim_time) + (int)apsim_timestep);
        ...
        private double Time(int yy, int dd, int tt)
        {
            ...
            double beginStartYear = DateUtilities.DateTimeToJulianDayNumber(new DateTime(clock.StartDate.Year, 1, 1)) - 1.0;
            double julianStartDate = beginStartYear + clock.StartDate.DayOfYear - 1.0;
        ...
        private void DoSoilWaterMovement(object sender, EventArgs e)
        {
            int timeOfDay = TimeToMins(apsim_time);
            double timestepStart = Time(clock.Today.Year, clock.Today.DayOfYear, timeOfDay);
    ...
    loop:
        ...
        private void PStat(int istat, ref double tresp)
        {
            ...
            if (istat == 0)
            {
                ...
                double start_of_day = Time(clock.Today.Year, clock.Today.Day, TimeToMins(apsim_time));
                double end_of_day = Time(clock.Today.Year, clock.Today.Day, TimeToMins(apsim_time) + (int)apsim_timestep);
