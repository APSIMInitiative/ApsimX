        ...
        public override string FilterString(bool htmltags)
        {
            ...
            if (trueFalse | (model.AllPropertyInfoFound != null && model.AllPropertyInfoFound.Last().PropertyType.IsEnum))
            {
                if (model.AllPropertyInfoFound.Last().PropertyType == typeof(bool))
                {
                    ...
                    filterWriter.Write($" {CLEMModel.DisplaySummaryValueSnippet(model.PropertyOfIndividual, "Not set", HTMLSummaryStyle.Filter, htmlTags: htmltags)}");
                }
                else
                {
                    filterWriter.Write($" {CLEMModel.DisplaySummaryValueSnippet(model.PropertyOfIndividual, "Not set", HTMLSummaryStyle.Filter, htmlTags: htmltags)}");
                    ...
                    filterWriter.Write($" {CLEMModel.DisplaySummaryValueSnippet(model.Value?.ToString(), "No value", HTMLSummaryStyle.Filter, htmlTags: htmltags)}");
                }
            }
            else
            {
                ...
                if (model.AllPropertyInfoFound != null)
                {
                    if (model.IsOperatorValid)
                    {
                        filterWriter.Write($" {CLEMModel.DisplaySummaryValueSnippet(model.OperatorToSymbol(), "Unknown operator", HTMLSummaryStyle.Filter, htmlTags: htmltags)}");
                    }
                    else
                    {
                        string errorLink = (htmltags) ? "<span class=\"errorlink\">" : "";
