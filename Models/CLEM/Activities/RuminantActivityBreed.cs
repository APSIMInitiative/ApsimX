    ...
    public class RuminantActivityBreed : CLEMRuminantActivityBase
    {
        ...
        private void OnCLEMInitialiseActivity(object sender, EventArgs e)
        {
            ...
            foreach (RuminantFemale female in herd.OfType<RuminantFemale>().Where(a => a.SucklingOffspringList.Any()))
            {
                ...
                female.BreedDetails.OnConceptionStatusChanged(conceptionArgs);
...
//                        if (breeders.Any()) // & (!useControlledMating || controlledMating.TimingCheck(previousDate)))
//                        {
...
//                                        {
//                                            // get times suitable for this breeder
...
//                        }
            ...
            foreach (RuminantFemale female in pregnantherd)
            {
                ...
                if (female.BirthDue)
                {
                    ...
                    for (int i = 0; i < female.CarryingCount; i++)
                    {
                        ...
                        Ruminant newSucklingRuminant = Ruminant.Create(female.Fetuses[i], female.BreedDetails, events.TimeStepStart, 0, female.CurrentBirthScalar, weight);
        ...
        private void SetFemaleMateAttributes(RuminantFemale female, KeyValuePair<string, IIndividualAttribute> femaleAttribute, IIndividualAttribute maleAttribute)
        {
            ...
            else
            {
                ...
                if (female.BreedDetails.IsMandatoryAttribute(femaleAttribute.Key))
                {
                    string errorMsg;
                    if (useControlledMating)
