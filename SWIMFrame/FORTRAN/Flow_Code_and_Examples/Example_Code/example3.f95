...
program example3
 ...
 ! for 10 days with precipitation of 1 cm/h on days 1 and 6. The potential
 ! evaporation rate is 0.05 cm/h throughout. There is vapour flow, but no sink
 ! term and no solute transport. This example sets parameter h0max to 1 cm to
 ! limit ponding to 1 cm. Extra water runs off.
 use soildata, only:dx,ths,gettbls,Sofh,hofS
 use flow, only:dSmax,h0max,solve
 ...
 open (unit=2,iostat=j,file="example3.out",status="replace",position="rewind")
 ...
 h0max=1.0 ! limit ponding to 1 cm - extra runs off
 ...
 do j=1,10
  ...
  win=win+qprec*(tf-ti)
  if (j==1) then
   write (2,"(f10.1,i10,10f10.4)") tf,nsteps,h0,runoff
   write (2,"(10f8.4)") S
  end if
  ti=tf
  if (j==5) then ! more precip
   qprec=1.0
  else
   qprec=0.0 ! no precip
  end if
 ...
 write (2,"(2f16.6,e16.2)") wp-wpi,infil-drn,win-(wp-wpi+h0+evap+drn+runoff)
