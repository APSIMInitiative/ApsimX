...
namespace Models
{
    ...
    public class AgPastureProto : Model, ICrop, ICanopy, IUptake
    {
        ...
        private double calcPlantExtractableN()	// not all minN is extractable
        {
            ...
            for (layer = 0; layer < Soil.Thickness.Length; layer++)
            {
                if (spDepth <= p_rootFrontier)
                {
                    ...
                    double swaf = 1.0;
                    double LLmm = soilCrop.LL[layer] * Soil.Thickness[layer];
                    swaf = (Soil.SoilWater.SWmm[layer] - LLmm) / (Soil.SoilWater.DULmm[layer] - LLmm);
                    swaf = Math.Max(0.0, Math.Min(swaf, 1.0));
