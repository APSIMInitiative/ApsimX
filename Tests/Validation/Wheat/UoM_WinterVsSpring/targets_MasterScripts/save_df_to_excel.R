...
#' @param sheet_name Character. The name of the worksheet. Defaults to \code{"Observed"}.
#'
#' @return Character. The full path to the saved Excel file.
#' @export
save_df_to_excel <- function(df, folder_path, file_name, sheet_name = "Observed") {
  
  # ---- 1. DEFENSIVE INTEGRITY CHECKS & NULL SHIELDS ----
  if (missing(df) || !is.data.frame(df) || nrow(df) == 0) {
    stop("Error [save_df_to_excel]: Attempted to save an empty or invalid dataframe.")
  }
  
  # NULL SHIELD: This stops the "argument is of length zero" error and tells you exactly what's wrong!
  if (missing(folder_path) || is.null(folder_path) || length(folder_path) == 0) {
    stop("QC FAILED [save_df_to_excel]: 'folder_path' evaluated to NULL. Check the spelling in your config list!")
  }
  if (missing(file_name) || is.null(file_name) || length(file_name) == 0) {
    stop("QC FAILED [save_df_to_excel]: 'file_name' evaluated to NULL. Check the spelling in your config list!")
  }
  if (missing(sheet_name) || is.null(sheet_name) || length(sheet_name) == 0) {
    sheet_name <- "Observed" # Safe fallback just in case
  }
