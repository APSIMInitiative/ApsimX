...
namespace Gtk.Sheet
{
    ...
    public class PagedDataProvider : IDataProvider
    {
        ...
        private void CreateTemporaryKeyset()
        {
            ...
            else if (connection is Firebird)
            {
                if (orderBy.Length > 0)
                    ...
                    sort = $"ORDER BY \"SimulationID\"";
                if ((connection as Firebird).FieldExists(tableName, "Clock.Today"))
                    sort += ", \"Clock.Today\"";

                sql = "RECREATE GLOBAL TEMPORARY TABLE \"keyset\" (\"rowid\" BIGINT, \"rowidx\" BIGINT generated by default as identity primary key) ON COMMIT PRESERVE ROWS";
                dataStore.GetDataUsingSql(sql);
        ...
        private string GetRollingCursorRowFilter(int from, int count)
        {
            ...
            {
                data = dataStore.GetDataUsingSql($"SELECT FIRST {count} SKIP {from} \"rowid\" FROM \"keyset\" ORDER BY \"rowidx\"");
            }
