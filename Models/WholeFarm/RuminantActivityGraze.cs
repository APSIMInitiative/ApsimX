...
namespace Models.WholeFarm
{
	...
	/// <summary>This activity dertemines how a ruminant group will graze</summary>
	/// <summary>It is designed to request food via a pasture store arbitrator</summary>
	/// <version>1.0</version>
...
//		Clock Clock = null;
		...
		private void OnWFRequestFeed(object sender, EventArgs e)
		{
			...
			IResourceType pastureStoreType = null; 
			if (FeedArbitratorName != "")
			{
				pastureStoreType = Arbitrators.GetByName(FeedArbitratorName) as IResourceType;
			}
			...
			foreach (PastureActivityManage pasture in Activities.Children.Where(a => a.GetType() == typeof(PastureActivityManage)))
			{
				if (FeedArbitratorName == "")
				{
					pastureStoreType = pasture.FeedType;
				}
				...
				activity.Name = $"Graze {pasture.FeedType.Name}";
				...
				if (pasture.FeedType.Amount > 0)
				{
					...
					foreach (Ruminant ind in herd.Where(a => a.Location == pasture.ID))
					{
						...
						pastureStoreType.Remove(freqest);
					}
				}
			}
		}
