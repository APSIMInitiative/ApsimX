...
namespace UserInterface.Presenters
{
    public class AzureJobDisplayPresenter : IPresenter, ICloudJobPresenter
    {
        ...
        {
            Presenter.ShowMessage(msg, Simulation.ErrorLevel.Error);             
        }
        ...
        {
            Presenter.ShowMessage(msg, Simulation.ErrorLevel.Information);
        }
        ...
        public void StopJobs(List<string> jobIds)
        {
            ...
            if (jobIds.Count < 1)
            {
                Presenter.ShowMessage("Unable to stop jobs: no jobs are selected", Simulation.ErrorLevel.Information);
                return;
            }
        ...
        public void DeleteJobs(List<string> jobIds)
        {
            if (jobIds.Count < 1)
            {
                Presenter.ShowMessage("Unable to delete jobs: no jobs are selected.", Simulation.ErrorLevel.Information);
                return;
            }
