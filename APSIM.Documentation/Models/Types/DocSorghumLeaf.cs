...
namespace APSIM.Documentation.Models.Types
{
    ...
    public class DocSorghumLeaf : DocGeneric
    {
        ...
        public override List<ITag> Document(int heading = 0)
        {
            ...
            subTags.Add(new Section("TranspirationEfficiency", AutoDocumentation.Document((model as SorghumLeaf).potentialBiomassTEFunction)));

            subTags.Add(new Paragraph($"Initial DM mass = {(model as SorghumLeaf).InitialDMWeight} gm^-2^"));

            subTags.Add(new Section("Nitrogen Demand", new Paragraph("The N demand is calculated as defined in NDemands, based on DM demand the N concentration of each biomass pool.")));

            subTags.Add(new Section("DM Retranslocation Factor", new Paragraph($"{model.Name} does not retranslocate non-structural DM.")));

            subTags.Add(new Section("Nitrogen Supply", new Paragraph($"{model.Name} does not reallocate N when senescence of the organ occurs.")));
