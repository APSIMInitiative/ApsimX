    ...
    public class LifeCyclePhase : Model
    {
        ...
        public void Process()
        {
            if (Cohorts?.Count > 500)  //Check cohort number are not becomming silly
                throw new Exception(this.FullPath + " has over 500 cohorts which is to many really.  This is why your simulation is slow and the data store is about to chuck it in.  Check your " + this.Parent.Name.ToString() + " model to ensure development and mortality are sufficient.");
            ...
            if (Cohorts != null)
            {
                ...
                foreach (Cohort c in Cohorts)
                {
                    ...
                    if (Migrants > 0)
                    {
                        if (MigrantDestinations.Count == 0)
                            throw new Exception(this.FullPath + " is predicting values for migration but has no MigrationDestinationPhase specified");
                        ...
                        if ((MtotalProportion > 1.001) && (Migrants > 0))
                            throw new Exception("The sum of ProportionOfMigrants values in " + this.FullPath + " ProgenyDestinationPhases is greater than 1.0");
                    }
                }

                // Add progeny into destination phase
                if ((ProgenyDestinations.Count == 0)&&(Progeny > 0))
                        throw new Exception(this.FullPath + " is predicting values for reproduction but has no ProgenyDestinationPhase specified");
                ...
                if (((PtotalProportion < 0.999) || (PtotalProportion > 1.001))&&(Progeny > 0))
                    throw new Exception("The sum of ProportionOfProgeny values in " + this.FullPath + " ProgenyDestinationPhases does not equal 1.0");
