...
public class DocStructure : DocGeneric
{
    ...
    public override List<ITag> Document(int none = 0)
    {
        ...
        if (AutoDocumentation.DocumentModel(model.FindChild<IModel>("phyllochron")) is List<ITag> phyllochronTags)
        {
        ...
        if (AutoDocumentation.DocumentModel(model.FindChild<IModel>("thermalTime")) is List<ITag> thermalTimeTags)
        {
            leafTags.Add(new Paragraph("*ThermalTime* is given by"));
            leafTags.AddRange(thermalTimeTags);
        }

        if (AutoDocumentation.DocumentModel(model.FindChild<IModel>("finalLeafNumber")) is List<ITag> finalLeafNumberTags)
        {
        ...
        if (AutoDocumentation.DocumentModel(model.FindChild<IModel>("branchingRate")) is List<ITag> branchingRateTags)
        {
        ...
        if (AutoDocumentation.DocumentModel(model.FindChild<IModel>("branchMortality")) is List<ITag> branchMortalityTags)
        {
        ...
        if (AutoDocumentation.DocumentModel(model.FindChild<IModel>("heightModel")) is List<ITag> heightModelTags)
        {
