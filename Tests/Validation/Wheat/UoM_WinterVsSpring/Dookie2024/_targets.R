...
list(
  ...
  # format = "file" tells targets to compute a hash of the actual file contents.
  # If any file changes, this target invalidates.
  tar_target(
    name = tracked_excel_files,
    command = list.files(config$folder_observed, pattern = "\\.xlsx$", full.names = TRUE),
    format = "file"
  ),
  
  # 2. THE ZIPPER: Only runs if 'tracked_excel_files' detects a change.
  # It returns the zip file path, and format = "file" tells targets to track 
  # the resulting zip file artifact.
  tar_target(
    name = encrypted_zip_artifact,
    command = {
      # We put tracked_excel_files here just to force the dependency link in targets,
      # even though the function relies on the folder name itself.
