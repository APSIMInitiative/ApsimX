...
namespace UnitTests
{
    public class SimulationComparisonTests
    {
        ...
        public void CompareSimulationsOnSingleMatchField()
        {
            ...
            var columnNames = new List<string>() { "CheckpointID", "SimulationID", "Year", "Col1", "Col2" };

            var predictedRows = new List<object[]>
            {
                new object[] { 1, 1, 1970, 10, 11 },
                new object[] { 1, 1, 1971, 12, 13 },
                new object[] { 1, 2, 1970, 14, 15 },
                new object[] { 1, 2, 1971, 16, 17 }
            };
            database.InsertRows("Report", columnNames, predictedRows);

            var dataStore = new DataStore(database);

            SimulationComparison tool = new SimulationComparison();
            tool.TableName = "Report";
            tool.FieldNamesUsedForMatch = new string[] { "Year" };
        ...
        private static void CreateTable(IDatabaseConnection database)
        {
            ...
            columnNames = new List<string>() { "CheckpointID", "SimulationID", "Year", "Col1", "Col2" };
            columnTypes = new List<string>() { "integer", "integer", "integer", "integer", "real" };
