...
namespace UserInterface.Presenters
{
    ...
    public class ContextMenu
    {
        ...
        /// <param name="sender">Sender of the event</param>
        /// <param name="e">Event arguments</param>
        [ContextMenu(MenuName = "Run APSIM Dev",
                     AppliesTo = new Type[] { typeof(Simulation),
                                              typeof(Simulations),
                                              typeof(Experiment),
                                              typeof(Folder) },
                     ShortcutKey = Keys.F4)]
        public void RunAPSIMDev(object sender, EventArgs e)
        {
            if (this.explorerPresenter.Save())
            {
                Model model = Apsim.Get(this.explorerPresenter.ApsimXFile, this.explorerPresenter.CurrentNodePath) as Model;

                List<Model> PMFModels = new List<Model>();
                findAllPMFModels(model, PMFModels);

                string binFolder = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);

                string apsimResourceFilePath = Path.Combine(binFolder, "..", "Models", "Resources", PMFModels[0].Name + ".xml");

                //Overwrite the resource file
                XmlDocument xml = new XmlDocument();
                xml.Load(apsimResourceFilePath);

                Plant newPlant = (Plant)XmlUtilities.Deserialise(xml.DocumentElement, typeof(Models.PMF.Plant));

                updateAllPMFModels(model, newPlant);
        ...
        /// <param name="PMFModels"></param>
        void findAllPMFModels(Model model, List<Model> PMFModels)
        {
            for (int i = 0; i < model.Children.Count; i++)
            {
                if (model.Children[i].GetType() == typeof(Models.PMF.Plant))
                {
                    PMFModels.Add(model.Children[i]);
                }
                else if (model.Children[i].Children.Count > 0)
                {
                    findAllPMFModels(model.Children[i], PMFModels);
                }
            }
        }
        ...
        /// <param name="model"></param>
        /// <param name="PMFModels"></param>
        void updateAllPMFModels(Model model, Plant plant)
        {
            for (int i = 0; i < model.Children.Count; i++)
            {
                if (model.Children[i].GetType() == typeof(Models.PMF.Plant))
                {
                    model.Children[i] = plant;
                }
                else if (model.Children[i].Children.Count > 0)
                {
                    updateAllPMFModels(model.Children[i], plant);
                }
            }
        }

        /// <summary>
        /// Event handler for a User interface "Publish PMF" action
        /// </summary>
        /// <param name="sender">Sender of the event</param>
        /// <param name="e">Event arguments</param>
        [ContextMenu(MenuName = "Publish PMF", AppliesTo = new Type[] { typeof(Plant), typeof(Plant15) })]
        public void PublishPMF(object sender, EventArgs e)
        {
            Plant currentPlant = Apsim.Get(this.explorerPresenter.ApsimXFile, this.explorerPresenter.CurrentNodePath) as Plant;

            string binFolder = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);

            //Overwrite the resource file
            string xml = XmlUtilities.Serialise(currentPlant, true);

            string apsimResourceFilePath = Path.Combine(binFolder, "..", "Models", "Resources", currentPlant.Name + ".xml");

            File.WriteAllText(apsimResourceFilePath, xml);
        }

        /// <summary>
        /// Event handler for a User interface "Refresh PMF" action
        /// </summary>
        /// <param name="sender">Sender of the event</param>
        /// <param name="e">Event arguments</param>
        [ContextMenu(MenuName = "Refresh PMF", AppliesTo = new Type[] { typeof(Plant), typeof(Plant15) })]
        public void RefeshPMF(object sender, EventArgs e)
        {
            ...
            this.explorerPresenter.Delete(currentPlant);

            this.explorerPresenter.Add(File.ReadAllText(apsimResourceFilePath), getNodeParent(this.explorerPresenter.CurrentNodePath));
        }

        /// <summary>
        /// 
        /// </summary>
        /// <param name="currentNodePath"></param>
        private string getNodeParent(string currentNodePath)
        {
            return currentNodePath.Substring(0, currentNodePath.LastIndexOf("."));
        }
