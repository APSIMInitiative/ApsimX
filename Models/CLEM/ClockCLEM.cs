...
namespace Models.CLEM
{
    ...
    public class ClockCLEM : CLEMModel, IValidatableObject
    {
        /// <summary>
        /// Access to the Parent Clock 
        /// </summary>
        [Link] public Clock APSIMClock { get; set; }
        //[Link] private readonly Summary summary = null;
        ...
        [Core.Display(VisibleCallback = "IsCustomPropertyVisible")]
        ...
        protected virtual void OnStartOfSimulation(object sender, EventArgs e)
        {
            ...
            SetNextDate(APSIMClock.StartDate);
        ...
        private void SetNextDate(DateTime fromDate)
        {
            if (TimeStep == TimeStepTypes.Monthly)
            {
                ...
                Interval = (nextDate - APSIMClock.StartDate).Days;
            }
            else
            {
                nextDate = fromDate.AddDays(Interval);
        ...
        {
            return EcologicalIndicatorsNextDueDate.Year == APSIMClock.Today.Year && EcologicalIndicatorsNextDueDate.Month == APSIMClock.Today.Month;
        }
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            if (APSIMClock.StartDate.ToShortDateString() == "1/01/0001")
            {
                string[] memberNames = new string[] { "Clock.StartDate" };
                results.Add(new ValidationResult(String.Format("Invalid start date {0}", APSIMClock.StartDate.ToShortDateString()), memberNames));
            }
            if (APSIMClock.EndDate.ToShortDateString() == "1/01/0001")
            {
                string[] memberNames = new string[] { "Clock.EndDate" };
                results.Add(new ValidationResult(String.Format("Invalid end date {0}", APSIMClock.EndDate.ToShortDateString()), memberNames));
            }
            if (APSIMClock.StartDate.Day != 1)
            {
                string[] memberNames = new string[] { "Clock.StartDate" };
                results.Add(new ValidationResult(String.Format("CLEM must commence on the first day of a month. Invalid start date {0}", APSIMClock.StartDate.ToShortDateString()), memberNames));
            }
            return results;
        }
        ...
        private void OnCLEMValidate(object sender, EventArgs e)
        {
            ...
            if (APSIMClock.StartDate.Year > 1) // avoid checking if clock not set.
            {
                if ((int)EcologicalIndicatorsCalculationMonth >= APSIMClock.StartDate.Month)
                {
                    DateTime trackDate = new DateTime(APSIMClock.StartDate.Year, (int)EcologicalIndicatorsCalculationMonth, APSIMClock.StartDate.Day);
                    while (trackDate.AddMonths(-EcologicalIndicatorsCalculationInterval) >= APSIMClock.Today)
                        trackDate = trackDate.AddMonths(-EcologicalIndicatorsCalculationInterval);
                    EcologicalIndicatorsNextDueDate = trackDate;
                }
                else
                {
                    EcologicalIndicatorsNextDueDate = new DateTime(APSIMClock.StartDate.Year, (int)EcologicalIndicatorsCalculationMonth, APSIMClock.StartDate.Day);
                    while (APSIMClock.StartDate > EcologicalIndicatorsNextDueDate)
                        EcologicalIndicatorsNextDueDate = EcologicalIndicatorsNextDueDate.AddMonths(EcologicalIndicatorsCalculationInterval);
                }
            }
        }
