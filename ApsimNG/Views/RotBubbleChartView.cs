...
namespace UserInterface.Views
{
    ...
    public class RotBubbleChartView : ViewBase, IRotBubbleChartView
    {
        ...
        public EditorView RuleList = null;
        public EditorView ActionList = null;
        ...
        public RotBubbleChartView(ViewBase owner = null) : base(owner)
        {
            ...
            rules.Add(RuleList.MainWidget); RuleList.MainWidget.Show();
            ...
            actions.Add(ActionList.MainWidget); ActionList.MainWidget.Show();
        ...
        public void OnNameChanged(object o, EventArgs args)
        {
            ...
            OnGraphChanged?.Invoke(this, new GraphChangedEventArgs { model = Graph });
        }
...
// vvvvv REFACTOR THIS fixme
        ...
        private void PopulateMenus()
        {
            ...
            if (graphView.selectedObject == null )
            {
                ...
                handler = delegate (object s, EventArgs x)
                {
                    ...
                    Models.RotBubbleChart.StateNode newNode = new Models.RotBubbleChart.StateNode(n) { NodeName = n.Name };/* fixme set location to x,y of menu posting location, use IDs not names  */
            ...
            if (graphView.selectedObject?.GetType() == typeof(DGNode) && graphView.selected2Object == null)
            {
                ...
                handler = delegate (object s, EventArgs x)
                {
                    ...
                    AddNode?.Invoke(this, new AddNodeEventArgs { Node = new Models.RotBubbleChart.StateNode(n) { NodeName = newName } });
                    foreach (var arc in graphView.DirectedGraph.Arcs.FindAll(arc => arc.SourceName == graphView.selectedObject.Name))
                    {
                            ...
                            AddArc?.Invoke(this, new AddArcEventArgs { Arc = new Models.RotBubbleChart.RuleAction( newArc )});
                    }
            ...
            {
                item = new MenuItem("Duplicate Arc from " + (graphView.selectedObject as DGArc).sourceNode.Name + " to " + (graphView.selectedObject as DGArc).targetNode.Name);
                handler = delegate (object s, EventArgs x)
                {
                        Arc newArc = new Arc((graphView.selectedObject as DGArc).ToArc());
                        newArc.Name = graphView.DirectedGraph.nextArcID();
                        newArc.SourceName = (graphView.selectedObject as DGArc).sourceNode.Name;
                        newArc.DestinationName = (graphView.selectedObject as DGArc).targetNode.Name;
                        AddArc?.Invoke(this, new AddArcEventArgs { Arc = new Models.RotBubbleChart.RuleAction(newArc) });
                ...
                item = new MenuItem("Delete Arc from " + (graphView.selectedObject as DGArc).sourceNode.Name + " to " + (graphView.selectedObject as DGArc).targetNode.Name);
                handler = delegate (object s, EventArgs x)
                {
        ...
        private void OnGraphObjectSelected(object o, GraphObjectSelectedArgs args)
        {
            ...
            {
                AddNode(o, new AddNodeEventArgs { Node = new Models.RotBubbleChart.StateNode((args.Object1 as DGNode).ToNode()) { NodeName = NodeNames[(args.Object1 as DGNode).Name] } }); 
                OnSelect(selectMode.node, (args.Object1 as DGNode).Name);
                OnGraphChanged?.Invoke(this, new GraphChangedEventArgs { model = Graph });
            }
            if (args.Object1?.GetType() == typeof(DGArc))
            {
                AddArc(o, new AddArcEventArgs { Arc = new Models.RotBubbleChart.RuleAction((args.Object1 as DGArc).ToArc()) });
...
// ^^^^^ REFACTOR THIS        
        ...
        /// the embedded graph.
        /// </summary>
        public Models.RotBubbleChart.RBGraph Graph
        {
            get
            {
                Models.RotBubbleChart.RBGraph resultgraph = new Models.RotBubbleChart.RBGraph();
                DirectedGraph graph = graphView.DirectedGraph;
                graph.Nodes.ForEach(n =>
                {
                    resultgraph.Nodes.Add(new Models.RotBubbleChart.StateNode(n) { NodeName = NodeNames[n.Name] });
                });
                graph.Arcs.ForEach(a =>
                {
                    Arc ga = graph.Arcs.Find(x => x.Name == a.Name);
                    var na = new Models.RotBubbleChart.RuleAction(ga);
