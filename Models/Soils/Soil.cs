        ...
        public void Check(ISummary summary)
        {
            ...
            if (weirdo == null)
            {
                ...
                for (int layer = 0; layer < physical.Thickness.Length; layer++)
                {
                    ...
                    if (MathUtilities.GreaterThan(physical.ParticleSizeClay[layer] + physical.ParticleSizeSilt[layer] + physical.ParticleSizeSand[layer] - 100, 0, 0))
                        message.AppendLine($"Clay, silt and sand percentages in layer {layerNumber} add up to more than 100%");
                    else if (MathUtilities.LessThan(physical.ParticleSizeClay[layer] + physical.ParticleSizeSilt[layer] + physical.ParticleSizeSand[layer] - 100, 0, 0))
