...
program example8
 ...
 ! surface evaporation and continuous water extraction by roots, calculated in
 ! module "sinks" using module "roots". Drippers and drains are in different
 ! layers, so solve can calculate addition and extraction of water and solute
 ! using 2 water extraction streams, one for roots and one for drippers and
 ! drains.
 ...
 open (unit=2,iostat=j,file="example8.out",status="replace",position="rewind")
 ...
 call setsinks(n,ns,idrip,idrn,driprate,dripsol,dcond,jt)
 ...
 wex=0.0 ! cumulative water extraction
...
!   sum(wex),sp,sum(sum(sex,1),1)
 ...
 write (2,"(10f8.4)") S
 write (2,"(5e16.4)") h
 write (2,"(7g16.6)") wp,evap,infil,drn,sum(wex)
 write (2,"(2f16.6,e16.2)") wp-wpi,infil-drn,win-(wp-wpi+h0+evap+drn+sum(wex))
